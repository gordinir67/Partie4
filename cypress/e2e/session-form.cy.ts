describe('Session create/update screens', () => {
  const teachers = [
    { id: 1, firstName: 'Ben', lastName: 'Test', createdAt: '2025-01-01T00:00:00.000Z', updatedAt: '2025-01-02T00:00:00.000Z' },
    { id: 2, firstName: 'johanna', lastName: 'Noname', createdAt: '2025-01-01T00:00:00.000Z', updatedAt: '2025-01-02T00:00:00.000Z' },
  ];

  it('Create: Save disabled until all fields are filled', () => {
    cy.uiLoginAsAdmin([]);
    cy.intercept('GET', '/api/teacher', { statusCode: 200, body: teachers }).as('teachers');

    cy.visit('/sessions/create');
    cy.wait('@teachers');

    // Avoid brittle title assertions; focus on functional elements.
    cy.get('input[formControlName=name]').should('be.visible');
    cy.contains('button', 'Save').should('be.disabled');
  });

  it('Create: creates a session and goes back to /sessions', () => {
    cy.uiLoginAsAdmin([]);
    ...
    cy.intercept('POST', '/api/session', (req) => {
      ...
      // date is sent as YYYY-MM-DD from the HTML input
      expect(req.body).to.have.property('date', '2026-02-01');

      req.reply({
        statusCode: 200,
        body: { id: 99, ...req.body, users: [], createdAt: '2026-02-01T00:00:00.000Z', updatedAt: '2026-02-01T00:00:00.000Z' },
      });
    }).as('create');

    // When the form navigates back to /sessions, the list requests GET /api/session again.
    cy.intercept('GET', '/api/session', { statusCode: 200, body: [] }).as('sessionsAfterCreate');
    ...
    cy.contains('mat-option', 'Bob Martin').click();
  ...
  it('Update: updates a session and goes back to /sessions', () => {
    cy.uiLoginAsAdmin([]);
    ...
    cy.intercept('GET', '/api/teacher', { statusCode: 200, body: teachers }).as('teachers');
    cy.intercept('GET', '/api/session/1', { statusCode: 200, body: existing }).as('detail');
    ...
    cy.intercept('PUT', '/api/session/1', (req) => {
      ...
      req.reply({ statusCode: 200, body: { ...existing, ...req.body, updatedAt: '2026-01-31T00:00:00.000Z' } });
    }).as('update');

    cy.intercept('GET', '/api/session', { statusCode: 200, body: [] }).as('sessionsAfterUpdate');
